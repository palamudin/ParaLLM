...
class AppRouteTests(unittest.TestCase):
    ...
    def test_run_contract_drawer_uses_pill_controls_and_summary_chips(self) -> None:
        ...
        self.assertIn('data-contract-control-tile="Engine"', response.text)
        self.assertIn('data-contract-control-tile="Baseline"', response.text)
        self.assertNotIn('class="igs-contract-telemetry"', response.text)
        self.assertIn('data-contract-pill-select="Worker model"', response.text)
        ...
        self.assertIn('data-composer-reasoning-option="xhigh"', response.text)
    ...
    def test_run_contract_uses_toggles_for_modes_and_boolean_controls(self) -> None:
        ...
        self.assertIn('data-composer-tool-action="web-search"', response.text)
        self.assertIn('data-composer-tool-action="memory"', response.text)
        self.assertIn('data-composer-tool-action="vetting"', response.text)
        self.assertIn('data-composer-reasoning-option="low"', response.text)
        self.assertIn('data-composer-reasoning-option="xhigh"', response.text)
        self.assertIn('data-state-label="', response.text)
        self.assertIn('class="igs-header-cycle', response.text)
